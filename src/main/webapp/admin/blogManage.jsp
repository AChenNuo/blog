...
<head>
    ...
    <script type="text/javascript">
        //返回该行的博客类型名称,当前这一行（row）中这条属性（field-》val）实际获得的值为实体类属性中的子属性值(val.typeName)
        ...
        function openBlogModifyTab() {
            ...
            //（查询的父窗口下打开），也就是此时打开的窗口与查询的窗口是同级的，并列的。
            var row = selectedRows[0];
            window.parent.openTab("修改博客","modifyBlog.jsp?id=" + row.id ,"icon-edit");
        }
        
        //查看一条博客信息openBlogLookTab()
        function openBlogLookTab() {
            var selectedRows = $("#dg").datagrid("getSelections");
            if (selectedRows.length != 1){
                $.messager.alert("系统提示","请选择一个要修改的博客！");
                return;
            ...
            var row = selectedRows[0];
            window.parent.openTab("查看博客","lookBlog.jsp?id=" + row.id ,"icon-search");
        }
        ...
        function deleteBlog() {
            ...
            if (selectedRows.length == 0){
                $.messager.alert("系统提示","请选择至少一条需要删除的博客");
                return;
            }
            var strIds = [];
            ...
            $.messager.confirm("系统提示","您确定要删除这<font color =red>" + selectedRows.length + "</font>条数据吗？",function (r) {
                ...
                //这里的json,规定预期的服务器响应的数据类型。"json" - 以 JSON 运行响应，并以 JavaScript 对象返回.
            });
        }
...
<div id="tb">
    <div>
        <a href="javascript:openBlogLookTab()" class="easyui-linkbutton" iconCls="icon-home" plain="true">查看</a>
